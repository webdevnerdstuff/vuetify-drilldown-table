...
const xa = { static: function() {
  ...
  function c() {
    if (d = !1, requestAnimationFrame(() => {
      ...
      return;
    const m = e.activatorEl.value.getBoundingClientRect(), h = function(C, w) {
      const F = ql(C);
      return w ? F.x += parseFloat(C.style.right || 0) : F.x -= parseFloat(C.style.left || 0), F.y -= parseFloat(C.style.top || 0), F;
    ...
    const f = k.reduce((C, w) => {
      const F = w.getBoundingClientRect(), D = new dt({ x: w === document.documentElement ? 0 : F.x, y: w === document.documentElement ? 0 : F.y, width: w.clientWidth, height: w.clientHeight });
    ...
    function S(C) {
      const w = new dt(h), F = _n(C.anchor, m), D = _n(C.origin, w);
      ...
      return w.x += B, w.y += A, w.width = Math.min(w.width, s.value), w.height = Math.min(w.height, i.value), { overflows: xn(w, f), x: B, y: A };
    }
    let y = 0, _ = 0;
      ...
      y += C, _ += w, h.x += C, h.y += w;
      {
        ...
        if (["x", "y"].forEach((L) => {
          if (L === "x" && B && !E.x || L === "y" && A && !E.y) {
            ...
            const { overflows: z } = S(N);
            (z[L].before <= F[L].before && z[L].after <= F[L].after || z[L].before + z[L].after < (F[L].before + F[L].after) / 2) && (b = N, I = E[L] = !0);
          }
        }), I)
          continue;
      }
      F.x.before && (y += F.x.before, h.x += F.x.before), F.x.after && (y -= F.x.after, h.x -= F.x.after), F.y.before && (_ += F.y.before, h.y += F.y.before), F.y.after && (_ -= F.y.after, h.y -= F.y.after);
      {
        const D = xn(h, f);
        V.x = f.width - D.x.before - D.x.after, V.y = f.height - D.y.before - D.y.after, y += D.x.before, h.x += D.x.before, _ += D.y.before, h.y += D.y.before;
      }
      break;
    }
    const P = wn(b.anchor);
    return Object.assign(t.value, { "--v-overlay-anchor-origin": `${b.anchor.side} ${b.anchor.align}`, transformOrigin: `${b.origin.side} ${b.origin.align}`, top: X(xl(_)), left: e.isRtl.value ? void 0 : X(xl(y)), right: e.isRtl.value ? X(xl(-y)) : void 0, minWidth: X(P === "y" ? Math.min(a.value, m.width) : a.value), maxWidth: X(Pn(xt(V.x, a.value === 1 / 0 ? 0 : a.value, s.value))), maxHeight: X(Pn(xt(V.y, r.value === 1 / 0 ? 0 : r.value, i.value))) }), { available: V, contentBox: h };
  }
  return J([e.activatorEl, e.contentEl], (m, h) => {
    let [k, f] = m, [b, S] = h;
    b && v.unobserve(b), k && v.observe(k), S && v.unobserve(S), f && v.observe(f);
  }, { immediate: !0 }), Pe(() => {
    v.disconnect();
...
const Dl = Symbol.for("vuetify:v-menu"), _r = M({ closeDelay: [Number, String], openDelay: [Number, String] }, "delay"), Pr = M({ activator: [String, Object], activatorProps: { type: Object, default: () => ({}) }, openOnClick: { type: Boolean, default: void 0 }, openOnHover: Boolean, openOnFocus: { type: Boolean, default: void 0 }, closeOnContentClick: Boolean, ..._r() }, "VOverlay-activator");
function Fr(e, l) {
  let { isActive: t, isTop: n } = l;
  const o = U();
  let a = !1, r = !1, s = !0;
  const i = g(() => e.openOnFocus || e.openOnFocus == null && e.openOnHover), u = g(() => e.openOnClick || e.openOnClick == null && !e.openOnHover && !i.value), { runOpenDelay: d, runCloseDelay: v } = function($, P) {
    const C = {}, w = (F) => () => {
      if (!Ie)
    ...
    var P;
    (P = $.sourceCapabilities) != null && P.firesTouchEvents || (a = !0, o.value = $.currentTarget || $.target, d());
  }, h = ($) => {
    a = !1, v();
  }, k = ($) => {
  ...
  return J(() => !!e.activator, ($) => {
    $ && Ie ? (E = Gl(), E.run(() => {
      (function(P, C, w) {
        let { activatorEl: F, activatorEvents: D } = w;
        function B() {
          let L = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : I(), N = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : P.activatorProps;
          L && function(T, z) {
        ...
        function I() {
          var T, z;
          let L, N = arguments.length > 0 && arguments[0] !== void 0 ? arguments[0] : P.activator;
          if (N)
            if (N === "parent") {
        ...
        J(() => P.activator, (L, N) => {
          ...
          L && Ee(() => B());
        }, { immediate: !0 }), J(() => P.activatorProps, () => {
          B();
        }), Pe(() => {
          A();
        });
    ...
    E == null || E.stop();
  }), { activatorEl: o, activatorRef: _, activatorEvents: b, contentEvents: S, scrimEvents: y };
}
function Ql(e) {
  return Ul(() => {
    const l = [], t = {};
...
const $n = Symbol.for("vuetify:stack"), St = Pt([]), Vn = Symbol.for("vuetify:theme"), be = M({ theme: String }, "theme");
function Ce(e) {
  Se("provideTheme");
  const l = fe(Vn, null);
  if (!l)
...
const _a = M({ absolute: Boolean, attach: [Boolean, String, Object], closeOnBack: { type: Boolean, default: !0 }, contained: Boolean, contentClass: null, contentProps: null, disabled: Boolean, noClickAnimation: Boolean, modelValue: Boolean, persistent: Boolean, scrim: { type: [String, Boolean], default: !0 }, zIndex: { type: [Number, String], default: 2e3 }, ...Pr(), ...se(), ...Et(), ...Vr(), ...Cr(), ...Br(), ...be(), ...Dt() }, "VOverlay"), On = q()({ name: "VOverlay", directives: { ClickOutside: Ir }, inheritAttrs: !1, props: { _disableGlobalStack: Boolean, ..._a() }, emits: { "click:outside": (e) => !0, "update:modelValue": (e) => !0, afterLeave: () => !0 }, setup(e, l) {
  var j, W;
  ...
  const { themeClasses: u } = Ce(e), { rtlClasses: d, isRtl: v } = Ot(), { hasContent: c, onAfterLeave: m } = function(O, G) {
    ...
    const K = Se("useStack"), le = !R, ee = fe($n, void 0), Q = Pt({ activeChildren: /* @__PURE__ */ new Set() });
    ge($n, Q);
    const ce = ue(+G.value);
    lt(O, () => {
      var bt;
      const Me = (bt = St.at(-1)) == null ? void 0 : bt[1];
      ce.value = Me ? Me + 10 : +G.value, le && St.push([K.uid, ce.value]), ee == null || ee.activeChildren.add(K.uid), Pe(() => {
        if (le) {
  ...
  }(r, H(e, "zIndex"), e._disableGlobalStack), { activatorEl: S, activatorRef: y, activatorEvents: _, contentEvents: V, scrimEvents: E } = Fr(e, { isActive: r, isTop: f }), { dimensionStyles: $ } = It(e), P = $r(), { scopeId: C } = Ca();
  J(() => e.disabled, (O) => {
    O && (r.value = !1);
  });
  const w = U(), F = U(), { contentStyles: D, updateLocation: B } = function(O, G) {
    const R = U({}), K = U();
    function le(ee) {
      var Q;
    ...
    }), window.addEventListener("resize", le, { passive: !0 }), Pe(() => {
      window.removeEventListener("resize", le), K.value = void 0;
    })), { contentStyles: R, updateLocation: K };
  }(e, { isRtl: v, contentEl: F, activatorEl: S, isActive: r });
  function A(O) {
  ...
  function L(O) {
    var G, R;
    O.key === "Escape" && k.value && (e.persistent ? z() : (r.value = !1, (G = F.value) != null && G.contains(document.activeElement) && ((R = S.value) == null || R.focus())));
  }
  (function(O, G) {
    if (!Ie)
      return;
  ...
  })(e, { root: w, contentEl: F, activatorEl: S, isActive: r, updateLocation: B }), Ie && J(r, (O) => {
  ...
  return J(() => r.value && (e.absolute || e.contained) && s.value == null, (O) => {
    ...
    var O;
    return p(re, null, [(O = t.activator) == null ? void 0 : O.call(t, { isActive: r.value, props: Z({ ref: y }, _.value, e.activatorProps) }), P.value && p(cr, { disabled: !s.value, to: s.value }, { default: () => [c.value && p("div", Z({ class: ["v-overlay", { "v-overlay--absolute": e.absolute || e.contained, "v-overlay--active": r.value, "v-overlay--contained": e.contained }, u.value, d.value, e.class], style: [b.value, { top: X(T.value) }, e.style], ref: w }, C, n), [p(Or, Z({ color: h, modelValue: r.value && !!e.scrim }, E.value), null), p(Ke, { appear: !0, persisted: !0, transition: e.transition, target: S.value, onAfterLeave: () => {
      m(), o("afterLeave");
    } }, { default: () => {
      var G;
      return [Ae(p("div", Z({ ref: F, class: ["v-overlay__content", e.contentClass], style: [$.value, D.value] }, V.value, e.contentProps), [(G = t.default) == null ? void 0 : G.call(t, { isActive: r })]), [[yt, r.value], [Ye("click-outside"), { handler: A, closeConditional: I, include: () => [S.value] }]])];
    } })])] })]);
  }), { activatorEl: S, animateClick: z, contentEl: F, globalTop: k, localTop: f, updateLocation: B };
        ...
        for (let S = 0; S < d.length; S++) {
          ...
          if (v && !(c != null && c.noFilter)) {
            if (typeof y == "object") {
              ...
              for (const w of C) {
                const F = ke(y, w, y), D = (b = c == null ? void 0 : c.customKeyFilter) == null ? void 0 : b[w];
            ...
            const $ = Object.keys(V).length, P = Object.keys(_).length;
            if (!$ && !P || (c == null ? void 0 : c.filterMode) === "union" && P !== f && !$ || (c == null ? void 0 : c.filterMode) === "intersection" && (P !== f || !$))
              continue;
          }
          m.push({ index: S, matches: { ...V, ..._ } });
        }
      return m;
...
const we = M({ tag: { type: String, default: "div" } }, "tag"), Mr = M({ expandOnClick: Boolean, showExpand: Boolean, expanded: { type: Array, default: () => [] } }, "DataTable-expand"), Pa = Symbol.for("vuetify:datatable:expanded");
function Fa(e) {
...
function Qa(e, l) {
  let t = !1;
  const n = Pt([]), o = ve(e, "modelValue", [], (u) => u == null ? [] : Tn(n, Oe(u)), (u) => {
...
const de = [String, Function, Object, Array], Ur = Symbol.for("vuetify:icons"), el = M({ icon: { type: de }, tag: { type: String, required: !0 } }, "icon"), Ln = q()({ name: "VComponentIcon", props: el(), setup(e, l) {
  let { slots: t } = l;
  return () => {
    const n = e.icon;
...
const Xr = M({ color: String, start: Boolean, end: Boolean, icon: de, ...se(), ...Rt(), ...we({ tag: "i" }), ...be() }, "VIcon"), he = q()({ name: "VIcon", props: Xr(), setup(e, l) {
  ...
  const n = 2 * Math.PI * 20, o = U(), { themeClasses: a } = Ce(e), { sizeClasses: r, sizeStyles: s } = jt(e), { textColorClasses: i, textColorStyles: u } = je(H(e, "color")), { textColorClasses: d, textColorStyles: v } = je(H(e, "bgColor")), { intersectionRef: c, isIntersecting: m } = to(), { resizeRef: h, contentRect: k } = function(E) {
    ...
    if (Ie) {
      const C = new ResizeObserver((w) => {
        E == null || E(w, C), w.length && (P.value = w[0].contentRect);
      });
      at(() => {
        C.disconnect();
      }), J($, (w, F) => {
        F && (C.unobserve(Al(F)), P.value = void 0), w && C.observe(Al(w));
      }, { flush: "post" });
    }
    return { resizeRef: $, contentRect: zl(P) };
  }(), f = g(() => Math.max(0, Math.min(100, parseFloat(e.modelValue)))), b = g(() => Number(e.width)), S = g(() => s.value ? Number(e.size) : k.value ? k.value.width : Math.max(b.value, 32)), y = g(() => 20 / (1 - b.value / S.value) * 2), _ = g(() => b.value / S.value * y.value), V = g(() => X((100 - f.value) / 100 * n));
  return qe(() => {
    c.value = o.value, h.value = o.value;
  }), te(() => p(e.tag, { ref: o, class: ["v-progress-circular", { "v-progress-circular--indeterminate": !!e.indeterminate, "v-progress-circular--visible": m.value, "v-progress-circular--disable-shrink": e.indeterminate === "disable-shrink" }, a.value, r.value, i.value, e.class], style: [s.value, u.value, e.style], role: "progressbar", "aria-valuemin": "0", "aria-valuemax": "100", "aria-valuenow": e.indeterminate ? void 0 : f.value }, { default: () => [p("svg", { style: { transform: `rotate(calc(-90deg + ${Number(e.rotate)}deg))` }, xmlns: "http://www.w3.org/2000/svg", viewBox: `0 0 ${y.value} ${y.value}` }, [p("circle", { class: ["v-progress-circular__underlay", d.value], style: v.value, fill: "transparent", cx: "50%", cy: "50%", r: 20, "stroke-width": _.value, "stroke-dasharray": n, "stroke-dashoffset": 0 }, null), p("circle", { class: "v-progress-circular__overlay", fill: "transparent", cx: "50%", cy: "50%", r: 20, "stroke-width": _.value, "stroke-dasharray": n, "stroke-dashoffset": V.value }, null)]), t.default && p("div", { class: "v-progress-circular__content" }, [t.default({ value: f.value })])] })), {};
...
const tl = { show(e, l) {
  ...
  const { radius: a, scale: r, x: s, y: i, centerX: u, centerY: d } = function(h, k) {
    var $;
    let f = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : {}, b = 0, S = 0;
    if (!so(h)) {
      const P = k.getBoundingClientRect(), C = Ll(h) ? h.touches[h.touches.length - 1] : h;
      b = C.clientX - P.left, S = C.clientY - P.top;
    }
    let y = 0, _ = 0.3;
    ($ = k._ripple) != null && $.circle ? (_ = 0.15, y = k.clientWidth / 2, y = f.center ? y : y + Math.sqrt((b - y) ** 2 + (S - y) ** 2) / 4) : y = Math.sqrt(k.clientWidth ** 2 + k.clientHeight ** 2) / 2;
    const V = (k.clientWidth - 2 * y) / 2 + "px", E = (k.clientHeight - 2 * y) / 2 + "px";
    return { radius: y, scale: _, x: f.center ? V : b - y + "px", y: f.center ? E : S - y + "px", centerX: V, centerY: E };
  }(e, l, t), v = 2 * a + "px";
...
const ml = { mounted: function(e, l) {
...
} }, ls = M({ active: { type: Boolean, default: void 0 }, symbol: { type: null, default: eo }, flat: Boolean, icon: [Boolean, String, Function, Object], prependIcon: de, appendIcon: de, block: Boolean, stacked: Boolean, ripple: { type: [Boolean, Object], default: !0 }, text: String, ...Tt(), ...se(), ...He(), ...Et(), ...Mt(), ...Ja(), ...nn(), ...no(), ...es(), ...Ze(), ...tn(), ...Rt(), ...we({ tag: "button" }), ...be(), ...rt({ variant: "elevated" }) }, "VBtn"), ut = q()({ name: "VBtn", directives: { Ripple: ml }, props: ls(), emits: { "group:selected": (e) => !0 }, setup(e, l) {
  let { attrs: t, slots: n } = l;
  const { themeClasses: o } = Ce(e), { borderClasses: a } = Lt(e), { colorClasses: r, colorStyles: s, variantClasses: i } = fl(e), { densityClasses: u } = We(e), { dimensionStyles: d } = It(e), { elevationClasses: v } = Nt(e), { loaderClasses: c } = an(e), { locationStyles: m } = ao(e), { positionClasses: h } = function(C) {
    let w = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : ze();
    return { positionClasses: g(() => C.position ? `${w}--${C.position}` : void 0) };
  }(e), { roundedClasses: k } = Qe(e), { sizeClasses: f, sizeStyles: b } = jt(e), S = Za(e, e.symbol, !1), y = en(e, t), _ = g(() => {
    var C;
    return e.active !== void 0 ? e.active : y.isLink.value ? (C = y.isActive) == null ? void 0 : C.value : S == null ? void 0 : S.isSelected.value;
  }), V = g(() => (S == null ? void 0 : S.disabled.value) || e.disabled), E = g(() => e.variant === "elevated" && !(e.disabled || e.flat || e.border)), $ = g(() => {
    if (e.value !== void 0)
      return Object(e.value) === e.value ? JSON.stringify(e.value, null, 0) : e.value;
  });
  function P(C) {
    var w;
    V.value || ((w = y.navigate) == null || w.call(y, C), S == null || S.toggle());
  }
  return function(C, w) {
    J(() => {
      var F;
...
} }), go = Symbol.for("vuetify:selection-control-group"), yo = M({ color: String, disabled: Boolean, defaultsTarget: String, error: Boolean, id: String, inline: Boolean, falseIcon: de, trueIcon: de, ripple: { type: Boolean, default: !0 }, multiple: { type: Boolean, default: null }, name: String, readonly: Boolean, modelValue: null, type: String, valueComparator: { type: Function, default: ot }, ...se(), ...He(), ...be() }, "SelectionControlGroup"), as = M({ ...yo({ defaultsTarget: "VSelectionControl" }) }, "VSelectionControlGroup");
...
const ho = M({ label: String, trueValue: null, falseValue: null, value: null, ...se(), ...yo() }, "VSelectionControl"), os = q()({ name: "VSelectionControl", directives: { Ripple: ml }, inheritAttrs: !1, props: ho(), emits: { "update:modelValue": (e) => !0 }, setup(e, l) {
  ...
  const { group: o, densityClasses: a, icon: r, model: s, textColorClasses: i, textColorStyles: u, trueValue: d } = function(y) {
    const _ = fe(go, void 0), { densityClasses: V } = We(y), E = ve(y, "modelValue"), $ = g(() => y.trueValue !== void 0 ? y.trueValue : y.value === void 0 || y.value), P = g(() => y.falseValue !== void 0 && y.falseValue), C = g(() => !!y.multiple || y.multiple == null && Array.isArray(E.value)), w = g({ get() {
      const A = _ ? _.modelValue.value : E.value;
      return C.value ? A.some((I) => y.valueComparator(I, $.value)) : y.valueComparator(A, $.value);
    }, set(A) {
      if (y.readonly)
        return;
      const I = A ? $.value : P.value;
      let L = I;
      C.value && (L = A ? [...Oe(E.value), I] : Oe(E.value).filter((N) => !y.valueComparator(N, $.value))), _ ? _.modelValue.value = L : E.value = L;
    } }), { textColorClasses: F, textColorStyles: D } = je(g(() => !w.value || y.error || y.disabled ? void 0 : y.color)), B = g(() => w.value ? y.trueIcon : y.falseIcon);
    return { group: _, densityClasses: V, trueValue: $, falseValue: P, model: w, textColorClasses: F, textColorStyles: D, icon: B };
  ...
  return o == null || o.onForceUpdate(() => {
    ...
    var E, $;
    const y = n.label ? n.label({ label: e.label, props: { for: c.value } }) : e.label, [_, V] = Kl(t);
    return p("div", Z({ class: ["v-selection-control", { "v-selection-control--dirty": s.value, "v-selection-control--disabled": e.disabled, "v-selection-control--error": e.error, "v-selection-control--focused": m.value, "v-selection-control--focus-visible": h.value, "v-selection-control--inline": e.inline }, a.value, e.class] }, _, { style: e.style }), [p("div", { class: ["v-selection-control__wrapper", i.value], style: u.value }, [(E = n.default) == null ? void 0 : E.call(n), Ae(p("div", { class: ["v-selection-control__input"] }, [r.value && p(he, { key: "icon", icon: r.value }, null), p("input", Z({ ref: k, checked: s.value, disabled: e.disabled, id: c.value, onBlur: b, onFocus: f, onInput: S, "aria-disabled": e.readonly, type: e.type, value: d.value, name: e.name, "aria-checked": e.type === "checkbox" ? s.value : void 0 }, V), null), ($ = n.input) == null ? void 0 : $.call(n, { model: s, textColorClasses: i, textColorStyles: u, props: { onFocus: f, onBlur: b, id: c.value } })]), [[Ye("ripple"), e.ripple && [!e.disabled && !e.readonly, null, ["center", "circle"]]]])]), y && p(mo, { for: c.value, clickable: !0 }, { default: () => [y] })]);
  }), { isFocused: m, input: k };
} }), bo = M({ indeterminate: Boolean, indeterminateIcon: { type: de, default: "$checkboxIndeterminate" }, ...ho({ falseIcon: "$checkboxOff", trueIcon: "$checkboxOn" }) }, "VCheckboxBtn"), mt = q()({ name: "VCheckboxBtn", props: bo(), emits: { "update:modelValue": (e) => !0, "update:indeterminate": (e) => !0 }, setup(e, l) {
  let { slots: t } = l;
...
const us = M({ disabled: { type: Boolean, default: null }, error: Boolean, errorMessages: { type: [Array, String], default: () => [] }, maxErrors: { type: [Number, String], default: 1 }, name: String, label: String, readonly: { type: Boolean, default: null }, rules: { type: Array, default: () => [] }, modelValue: null, validateOn: String, validationValue: null, ...wo() }, "validation"), rn = M({ id: String, appendIcon: de, centerAffix: { type: Boolean, default: !0 }, prependIcon: de, hideDetails: [Boolean, String], hint: String, persistentHint: Boolean, messages: { type: [Array, String], default: () => [] }, direction: { type: String, default: "horizontal", validator: (e) => ["horizontal", "vertical"].includes(e) }, "onClick:prepend": Te(), "onClick:append": Te(), ...se(), ...He(), ...us() }, "VInput"), ll = q()({ name: "VInput", props: { ...rn() }, emits: { "update:modelValue": (e) => !0 }, setup(e, l) {
  let { attrs: t, slots: n, emit: o } = l;
  const { densityClasses: a } = We(e), { rtlClasses: r } = Ot(), { InputIcon: s } = So(e), i = Le(), u = g(() => e.id || `input-${i}`), d = g(() => `${u.value}-messages`), { errorMessages: v, isDirty: c, isDisabled: m, isReadonly: h, isPristine: k, isValid: f, isValidating: b, reset: S, resetValidation: y, validate: _, validationClasses: V } = function(P) {
    let C = arguments.length > 1 && arguments[1] !== void 0 ? arguments[1] : ze(), w = arguments.length > 2 && arguments[2] !== void 0 ? arguments[2] : Le();
    const F = ve(P, "modelValue"), D = g(() => P.validationValue === void 0 ? F.value : P.validationValue), B = xo(), A = U([]), I = ue(!0), L = g(() => !(!Oe(F.value === "" ? null : F.value).length && !Oe(D.value === "" ? null : D.value).length)), N = g(() => !!(P.disabled ?? (B == null ? void 0 : B.isDisabled.value))), T = g(() => !!(P.readonly ?? (B == null ? void 0 : B.isReadonly.value))), z = g(() => P.errorMessages.length ? Oe(P.errorMessages).slice(0, Math.max(0, +P.maxErrors)) : A.value), j = g(() => {
      let Q = (P.validateOn ?? (B == null ? void 0 : B.validateOn.value)) || "input";
      Q === "lazy" && (Q = "input lazy");
    ...
    }), W = g(() => !P.error && !P.errorMessages.length && (!P.rules.length || (I.value ? !A.value.length && !j.value.lazy || null : !A.value.length))), O = ue(!1), G = g(() => ({ [`${C}--error`]: W.value === !1, [`${C}--dirty`]: L.value, [`${C}--disabled`]: N.value, [`${C}--readonly`]: T.value })), R = g(() => P.name ?? x(w));
    function K() {
      F.value = null, Ee(le);
    }
    function le() {
  ...
  }(e, "v-input", u), E = g(() => ({ id: u, messagesId: d, isDirty: c, isDisabled: m, isReadonly: h, isPristine: k, isValid: f, isValidating: b, reset: S, resetValidation: y, validate: _ })), $ = g(() => {
    var P;
    return (P = e.errorMessages) != null && P.length || !k.value && v.value.length ? v.value : e.hint && (e.persistentHint || e.focused) ? e.hint : e.messages;
  });
  return te(() => {
    var D, B, A, I;
    const P = !(!n.prepend && !e.prependIcon), C = !(!n.append && !e.appendIcon), w = $.value.length > 0, F = !e.hideDetails || e.hideDetails === "auto" && (w || !!n.details);
    return p("div", { class: ["v-input", `v-input--${e.direction}`, { "v-input--center-affix": e.centerAffix }, a.value, r.value, V.value, e.class], style: e.style }, [P && p("div", { key: "prepend", class: "v-input__prepend" }, [(D = n.prepend) == null ? void 0 : D.call(n, E.value), e.prependIcon && p(s, { key: "prepend-icon", name: "prepend" }, null)]), n.default && p("div", { class: "v-input__control" }, [(B = n.default) == null ? void 0 : B.call(n, E.value)]), C && p("div", { key: "append", class: "v-input__append" }, [e.appendIcon && p(s, { key: "append-icon", name: "append" }, null), (A = n.append) == null ? void 0 : A.call(n, E.value)]), F && p("div", { class: "v-input__details" }, [p(ss, { id: d.value, active: w, messages: $.value }, { message: n.message }), (I = n.details) == null ? void 0 : I.call(n, E.value)])]);
  }), { reset: S, resetValidation: y, validate: _ };
  ...
  return te(() => {
    ...
    return p(ll, Z({ class: ["v-checkbox", e.class] }, d, c, { modelValue: o.value, "onUpdate:modelValue": (f) => o.value = f, id: u.value, focused: a.value, style: e.style }), { ...n, default: (f) => {
      let { id: b, messagesId: S, isDisabled: y, isReadonly: _ } = f;
      return p(mt, Z(h, { id: b.value, "aria-describedby": S.value, disabled: y.value, readonly: _.value }, v, { modelValue: o.value, "onUpdate:modelValue": (V) => o.value = V, onFocus: r, onBlur: s }), n);
    } });
  }), {};
...
const Co = { mounted: function(e, l) {
  ...
  function v(P) {
    if ((!e.eager || !P) && (!Zl || P || e.eager)) {
      ...
      u.value.src && Ee(() => {
        var C, w;
        if (t("loadstart", ((C = a.value) == null ? void 0 : C.currentSrc) || u.value.src), (w = a.value) == null ? void 0 : w.complete) {
  ...
  function c() {
    var P;
    h(), r.value = "loaded", t("load", ((P = a.value) == null ? void 0 : P.currentSrc) || u.value.src);
  }
  function m() {
    var P;
    r.value = "error", t("error", ((P = a.value) == null ? void 0 : P.currentSrc) || u.value.src);
  }
  function h() {
    const P = a.value;
  ...
  function f(P) {
    ...
    const w = () => {
      clearTimeout(k);
      const { naturalHeight: F, naturalWidth: D } = P;
      F || D ? (s.value = D, i.value = F) : P.complete || r.value !== "loading" || C == null ? (P.currentSrc.endsWith(".svg") || P.currentSrc.startsWith("data:image/svg+xml")) && (s.value = 1, i.value = 1) : k = window.setTimeout(w, C);
    };
    w();
  }
  const b = g(() => ({ "v-img__img--cover": e.cover, "v-img__img--contain": !e.cover })), S = () => {
    var w;
    if (!u.value.src || r.value === "idle")
      return null;
    const P = p("img", { class: ["v-img__img", b.value], src: u.value.src, srcset: u.value.srcset, alt: e.alt, sizes: e.sizes, ref: a, onLoad: c, onError: m }, null), C = (w = n.sources) == null ? void 0 : w.call(n);
    return p(Ke, { transition: e.transition, appear: !0 }, { default: () => [Ae(C ? p("picture", { class: "v-img__picture" }, [C, P]) : P, [[yt, r.value === "loaded"]])] });
  }, y = () => p(Ke, { transition: e.transition }, { default: () => [u.value.lazySrc && r.value !== "loaded" && p("img", { class: ["v-img__img", "v-img__img--preload", b.value], src: u.value.lazySrc, alt: e.alt }, null)] }), _ = () => n.placeholder ? p(Ke, { transition: e.transition, appear: !0 }, { default: () => [(r.value === "loading" || r.value === "error" && !n.error) && p("div", { class: "v-img__placeholder" }, [n.placeholder()])] }) : null, V = () => n.error ? p(Ke, { transition: e.transition, appear: !0 }, { default: () => [r.value === "error" && p("div", { class: "v-img__error" }, [n.error()])] }) : null, E = () => e.gradient ? p("div", { class: "v-img__gradient", style: { backgroundImage: `linear-gradient(${e.gradient})` } }, null) : null, $ = ue(!1);
  {
    const P = J(d, (C) => {
      C && (requestAnimationFrame(() => {
  ...
  return te(() => {
    ...
    return Ae(p(Gn, Z({ class: ["v-img", { "v-img--booting": !$.value }, e.class], style: e.style }, P, { aspectRatio: d.value, "aria-label": e.alt, role: e.alt ? "img" : void 0 }), { additional: () => p(re, null, [p(S, null, null), p(y, null, null), p(E, null, null), p(_, null, null), p(V, null, null)]), default: n.default }), [[Ye("intersect"), { handler: v, options: e.options }, null, { once: !0 }]]);
  }), { currentSrc: o, image: a, state: r, naturalWidth: s, naturalHeight: i };
} }), vs = M({ start: Boolean, end: Boolean, icon: de, image: String, ...se(), ...He(), ...Ze(), ...Rt(), ...we(), ...be(), ...rt({ variant: "flat" }) }, "VAvatar"), nl = q()({ name: "VAvatar", props: vs(), setup(e, l) {
  let { slots: t } = l;
...
const ys = { open: (e) => {
  ...
  const t = U(/* @__PURE__ */ new Map()), n = U(/* @__PURE__ */ new Map()), o = ve(e, "opened", e.opened, (v) => new Set(v), (v) => [...v.values()]), a = g(() => {
    ...
    switch (e.selectStrategy) {
      ...
      default:
        return ((v) => {
          const c = { select: (m) => {
            let { id: h, value: k, selected: f, children: b, parents: S } = m;
            h = Ue(h);
            const y = new Map(f), _ = [h];
            for (; _.length; ) {
              const E = _.shift();
              f.set(E, k ? "on" : "off"), b.has(E) && _.push(...b.get(E));
            }
            let V = S.get(h);
            for (; V; ) {
              const E = b.get(V), $ = E.every((C) => f.get(C) === "on"), P = E.every((C) => !f.has(C) || f.get(C) === "off");
              f.set(V, $ ? "on" : P ? "off" : "indeterminate"), V = S.get(V);
            }
            return v && !k && Array.from(f.entries()).reduce(($, P) => {
              let [C, w] = P;
              return w === "on" ? [...$, C] : $;
            }, []).length === 0 ? y : f;
          }, in: (m, h, k) => {
  ...
  return (() => {
    const n = fe(_t, Ao);
    ge(_t, { ...n, isGroupActivator: !0 });
  })(), () => {
    var n;
    return (n = t.default) == null ? void 0 : n.call(t);
  };
} }), xs = M({ activeColor: String, baseColor: String, color: String, collapseIcon: { type: de, default: "$collapse" }, expandIcon: { type: de, default: "$expand" }, prependIcon: de, appendIcon: de, fluid: Boolean, subgroup: Boolean, title: String, value: null, ...se(), ...we() }, "VListGroup"), Wn = q()({ name: "VListGroup", props: xs(), setup(e, l) {
  let { slots: t } = l;
  const { isOpen: n, open: o, id: a } = $o(H(e, "value"), !0), r = g(() => `v-list-group--id-${String(a.value)}`), s = Po(), { isBooted: i } = function() {
    const m = ue(!1);
    return At(() => {
...
} }), ks = fa("v-list-item-subtitle"), Cs = fa("v-list-item-title"), Bs = M({ active: { type: Boolean, default: void 0 }, activeClass: String, activeColor: String, appendAvatar: String, appendIcon: de, baseColor: String, disabled: Boolean, lines: String, link: { type: Boolean, default: void 0 }, nav: Boolean, prependAvatar: String, prependIcon: de, ripple: { type: [Boolean, Object], default: !0 }, subtitle: [String, Number, Boolean], title: [String, Number, Boolean], value: null, onClick: Te(), onClickOnce: Te(), ...Tt(), ...se(), ...He(), ...Et(), ...Mt(), ...Ze(), ...tn(), ...we(), ...be(), ...rt({ variant: "text" }) }, "VListItem"), al = q()({ name: "VListItem", directives: { Ripple: ml }, props: Bs(), emits: { click: (e) => !0 }, setup(e, l) {
  let { attrs: t, slots: n, emit: o } = l;
  const a = en(e, t), r = g(() => e.value ?? a.href.value), { select: s, isSelected: i, isIndeterminate: u, isGroupActivator: d, root: v, parent: c, openOnSelect: m } = $o(r, !1), h = Po(), k = g(() => {
    var T;
    return e.active !== !1 && (e.active || ((T = a.isActive) == null ? void 0 : T.value) || i.value);
  }), f = g(() => e.link !== !1 && a.isLink.value), b = g(() => !e.disabled && e.link !== !1 && (e.link || a.isClickable.value || e.value != null && !!h)), S = g(() => e.rounded || e.nav), y = g(() => e.color ?? e.activeColor), _ = g(() => ({ color: k.value ? y.value ?? e.baseColor : e.baseColor, variant: e.variant }));
  J(() => {
    var T;
  ...
  const { themeClasses: V } = Ce(e), { borderClasses: E } = Lt(e), { colorClasses: $, colorStyles: P, variantClasses: C } = fl(_), { densityClasses: w } = We(e), { dimensionStyles: F } = It(e), { elevationClasses: D } = Nt(e), { roundedClasses: B } = Qe(S), A = g(() => e.lines ? `v-list-item--${e.lines}-line` : void 0), I = g(() => ({ isActive: k.value, select: s, isSelected: i.value, isIndeterminate: u.value }));
  function L(T) {
    var z;
  ...
  return te(() => {
    ...
    var K, le;
    return h == null || h.updateHasPrepend(R), e.activeColor && (K = "active-color", le = ["color", "base-color"], le = Array.isArray(le) ? le.slice(0, -1).map((ee) => `'${ee}'`).join(", ") + ` or '${le.at(-1)}'` : `'${le}'`, ul(`[Vuetify UPGRADE] '${K}' is deprecated, use ${le} instead.`)), Ae(p(T, { class: ["v-list-item", { "v-list-item--active": k.value, "v-list-item--disabled": e.disabled, "v-list-item--link": b.value, "v-list-item--nav": e.nav, "v-list-item--prepend": !R && (h == null ? void 0 : h.hasPrepend.value), [`${e.activeClass}`]: e.activeClass && k.value }, V.value, E.value, $.value, w.value, D.value, A.value, B.value, C.value, e.class], style: [P.value, F.value, e.style], href: a.href.value, tabindex: b.value ? h ? -2 : 0 : void 0, onClick: L, onKeydown: b.value && !f.value && N }, { default: () => {
      var ee;
...
} }), _s = M({ color: String, inset: Boolean, sticky: Boolean, title: String, ...se(), ...we() }, "VListSubheader"), Ps = q()({ name: "VListSubheader", props: _s(), setup(e, l) {
  let { slots: t } = l;
  ...
  return _o(), () => {
    ...
    return ((n = t.default) == null ? void 0 : n.call(t)) ?? ((o = e.items) == null ? void 0 : o.map((a) => {
      ...
      if (i === "subheader")
        return ((h = t.subheader) == null ? void 0 : h.call(t, { props: s })) ?? p(Ps, s, null);
...
const Is = M({ baseColor: String, activeColor: String, activeClass: String, bgColor: String, disabled: Boolean, lines: { type: [Boolean, String], default: "one" }, nav: Boolean, ...bs({ selectStrategy: "single-leaf", openStrategy: "list" }), ...Tt(), ...se(), ...He(), ...Et(), ...Mt(), itemType: { type: String, default: "type" }, ...Eo(), ...Ze(), ...we(), ...be(), ...rt({ variant: "text" }) }, "VList"), Os = q()({ name: "VList", props: Is(), emits: { "update:selected": (e) => !0, "update:opened": (e) => !0, "click:open": (e) => !0, "click:select": (e) => !0 }, setup(e, l) {
  ...
  _o(), Je({ VListGroup: { activeColor: k, baseColor: f, color: b }, VListItem: { activeClass: H(e, "activeClass"), activeColor: k, baseColor: f, color: b, density: H(e, "density"), disabled: H(e, "disabled"), lines: H(e, "lines"), nav: H(e, "nav"), variant: H(e, "variant") } });
  const S = ue(!1), y = U();
  function _(C) {
    S.value = !0;
  }
  ...
  function E(C) {
    var w;
    S.value || C.relatedTarget && ((w = y.value) != null && w.contains(C.relatedTarget)) || P();
  }
  function $(C) {
    if (y.value) {
      if (C.key === "ArrowDown")
  ...
  return te(() => p(e.tag, { ref: y, class: ["v-list", { "v-list--disabled": e.disabled, "v-list--nav": e.nav }, o.value, a.value, s.value, i.value, d.value, h.value, v.value, e.class], style: [r.value, u.value, e.style], tabindex: e.disabled || S.value ? -1 : 0, role: "listbox", "aria-activedescendant": void 0, onFocusin: _, onFocusout: V, onFocus: E, onKeydown: $ }, { default: () => [p(Vo, { items: n.value }, t)] })), { open: c, select: m, focus: P };
} }), Ds = M({ id: String, ...Wl(_a({ closeDelay: 250, closeOnContentClick: !0, locationStrategy: "connected", openDelay: 300, scrim: !1, scrollStrategy: "reposition", transition: { component: ba } }), ["absolute"]) }, "VMenu"), Ts = q()({ name: "VMenu", props: Ds(), emits: { "update:modelValue": (e) => !0 }, setup(e, l) {
  let { slots: t } = l;
  ...
  function c(E, $) {
    var P, C;
    e.autofocus && E && ((C = (P = $[0].target) == null ? void 0 : P.focus) == null || C.call(P));
  }
  ...
  function _(E) {
    E.stopPropagation(), b(), Ee(() => {
      a.value = null, function($) {
        for (var P = arguments.length, C = new Array(P > 1 ? P - 1 : 0), w = 1; w < P; w++)
          C[w - 1] = arguments[w];
        if (Array.isArray($))
          for (const F of $)
            F(...C);
  ...
  function V(E) {
    var P;
    const $ = E.target;
    if (a.value = $.value, ((P = e.modelModifiers) == null ? void 0 : P.trim) && ["text", "search", "password", "tel", "url"].includes(e.type)) {
  ...
  return te(() => {
    const E = !!(o.counter || e.counter || e.counterValue), $ = !(!E && !o.details), [P, C] = Kl(t), [{ modelValue: w, ...F }] = ll.filterProps(e), [D] = function(B) {
      return Vt(B, Object.keys(Kn.props).filter((A) => !$l(A) && A !== "class" && A !== "style"));
    }(e);
    return p(ll, Z({ ref: m, modelValue: a.value, "onUpdate:modelValue": (B) => a.value = B, class: ["v-text-field", { "v-text-field--prefixed": e.prefix, "v-text-field--suffixed": e.suffix, "v-text-field--plain-underlined": ["plain", "underlined"].includes(e.variant) }, e.class], style: e.style }, P, F, { centerAffix: !v.value, focused: r.value }), { ...o, default: (B) => {
      let { id: A, isDisabled: I, isDirty: L, isReadonly: N, isValid: T } = B;
      return p(Kn, Z({ ref: h, onMousedown: S, onClick: y, "onClick:clear": _, "onClick:prependInner": e["onClick:prependInner"], "onClick:appendInner": e["onClick:appendInner"], role: "textbox" }, D, { id: A.value, active: f.value || L.value, dirty: L.value || e.dirty, disabled: I.value, focused: r.value, error: T.value === !1 }), { ...o, default: (z) => {
  ...
  let f, b = "";
  const S = g(() => e.hideSelected ? i.value.filter((B) => !m.value.some((A) => A === B)) : i.value), y = g(() => e.hideNoData && !i.value.length || e.readonly || (c == null ? void 0 : c.isReadonly.value)), _ = U();
  function V(B) {
  ...
  function $(B) {
    var L, N;
    if (e.readonly || c != null && c.isReadonly.value || (["Enter", " ", "ArrowDown", "ArrowUp", "Home", "End"].includes(B.key) && B.preventDefault(), ["Enter", "ArrowDown", " "].includes(B.key) && (s.value = !0), ["Escape", "Tab"].includes(B.key) && (s.value = !1), B.key === "Home" ? (L = _.value) == null || L.focus("first") : B.key === "End" && ((N = _.value) == null || N.focus("last")), e.multiple || !function(T) {
  ...
  function w(B) {
    var A;
    (A = _.value) != null && A.$el.contains(B.relatedTarget) || (s.value = !1);
  }
  function F() {
    var B;
  ...
  return te(() => {
    const B = !(!e.chips && !t.chip), A = !!(!e.hideNoData || S.value.length || t["prepend-item"] || t["append-item"] || t["no-data"]), I = v.value.length > 0, [L] = Rl.filterProps(e), N = I || !k.value && e.label && !e.persistentPlaceholder ? void 0 : e.placeholder;
    return p(Rl, Z({ ref: o }, L, { modelValue: v.value.map((T) => T.props.value).join(", "), "onUpdate:modelValue": (T) => {
      T == null && (v.value = []);
    }, focused: k.value, "onUpdate:focused": (T) => k.value = T, validationValue: v.externalValue, dirty: I, class: ["v-select", { "v-select--active-menu": s.value, "v-select--chips": !!e.chips, ["v-select--" + (e.multiple ? "multiple" : "single")]: !0, "v-select--selected": v.value.length, "v-select--selection-slot": !!t.selection }, e.class], style: e.style, readonly: !0, placeholder: N, "onClick:clear": V, "onMousedown:control": E, onBlur: w, onKeydown: $ }), { ...t, default: () => p(re, null, [p(Ts, Z({ ref: a, modelValue: s.value, "onUpdate:modelValue": (T) => s.value = T, activator: "parent", contentClass: "v-select__content", disabled: y.value, eager: e.eager, maxHeight: 310, openOnClick: !1, closeOnContentClick: !1, transition: e.transition, onAfterLeave: F }, e.menuProps), { default: () => [A && p(Os, { ref: _, selected: h.value, selectStrategy: e.multiple ? "independent" : "single-independent", onMousedown: (T) => T.preventDefault(), onKeydown: P, onFocusin: D, tabindex: "-1" }, { default: () => {
      var T, z, j;
      return [(T = t["prepend-item"]) == null ? void 0 : T.call(t), !S.value.length && !e.hideNoData && (((z = t["no-data"]) == null ? void 0 : z.call(t)) ?? p(al, { title: n(e.noDataText) }, null)), S.value.map((W, O) => {
        var R;
...
} }), cn = (Un = { align: { type: String, default: "start" }, fixed: Boolean, fixedOffset: [Number, String], height: [Number, String], lastFixed: Boolean, noPadding: Boolean, tag: String, width: [Number, String] }, _l = (e, l) => {
...
function No(e, l) {
  ...
  J(() => e.headers, () => {
    var m, h, k;
    const a = e.headers.length ? Array.isArray(e.headers[0]) ? e.headers : [e.headers] : [], r = a.flatMap((f, b) => f.map((S) => ({ column: S, row: b }))), s = a.length, i = { title: "", sortable: !1 }, u = { ...i, width: 48 };
    ...
    r.forEach((f) => {
      let { column: b, row: S } = f;
      const y = b.key;
      for (let _ = S; _ <= S + (b.rowspan ?? 1) - 1; _++)
        d[_].push({ ...b, key: y, fixedOffset: v[_], sortable: b.sortable ?? !!b.key }), v[_] += Number(b.width ?? 0);
    }), d.forEach((f) => {
    ...
    t.value = d.map((f) => {
      const b = [];
      for (const S of f)
        c.has(S.key) || (c.add(S.key), b.push(S));
      return b;
    }), n.value = d.at(-1) ?? [];
  }, { deep: !0, immediate: !0 });
...
const Ro = M({ color: String, sticky: Boolean, multiSort: Boolean, sortAscIcon: { type: de, default: "$sortAsc" }, sortDescIcon: { type: de, default: "$sortDesc" }, ...nn() }, "VDataTableHeaders"), rl = q()({ name: "VDataTableHeaders", props: Ro(), setup(e, l) {
  ...
  const { toggleSort: o, sortBy: a, isSorted: r } = function() {
    ...
    return _;
  }(), { someSelected: s, allSelected: i, selectAll: u, showSelectAll: d } = pl(), { columns: v, headers: c } = gl(), { loaderClasses: m } = an(e), h = (_, V) => {
    if (e.sticky || _.fixed)
      return { position: "sticky", zIndex: _.fixed ? 4 : e.sticky ? 3 : void 0, left: _.fixed ? X(_.fixedOffset) : void 0, top: e.sticky ? `calc(var(--v-table-header-height) * ${V})` : void 0 };
  };
  function k(_) {
    const V = a.value.find((E) => E.key === _.key);
    return V ? V.order === "asc" ? e.sortAscIcon : e.sortDescIcon : e.sortAscIcon;
  }
  const { backgroundColorClasses: f, backgroundColorStyles: b } = ft(e, "color"), S = g(() => ({ headers: c.value, columns: v.value, toggleSort: o, isSorted: r, sortBy: a.value, someSelected: s.value, allSelected: i.value, selectAll: u, getSortIcon: k, getFixedStyles: h })), y = (_) => {
    let { column: V, x: E, y: $ } = _;
    const P = V.key === "data-table-select" || V.key === "data-table-expand";
    return p(cn, { tag: "th", align: V.align, class: ["v-data-table__th", { "v-data-table__th--sortable": V.sortable, "v-data-table__th--sorted": r(V) }, m.value], style: { width: X(V.width), minWidth: X(V.width), ...h(V, $) }, colspan: V.colspan, rowspan: V.rowspan, onClick: V.sortable ? () => o(V) : void 0, lastFixed: V.lastFixed, noPadding: P }, { default: () => {
      var F;
      const C = `column.${V.key}`, w = { column: V, selectAll: u, isSorted: r, toggleSort: o, sortBy: a.value, someSelected: s.value, allSelected: i.value, getSortIcon: k };
      return t[C] ? t[C](w) : V.key === "data-table-select" ? ((F = t["column.data-table-select"]) == null ? void 0 : F.call(t, w)) ?? (d && p(mt, { modelValue: i.value, indeterminate: s.value && !i.value, "onUpdate:modelValue": u }, null)) : p("div", { class: "v-data-table-header__content" }, [p("span", null, [V.title]), V.sortable && p(he, { key: "icon", class: "v-data-table-header__sort-icon", icon: k(V) }, null), e.multiSort && r(V) && p("div", { key: "badge", class: ["v-data-table-header__sort-badge", ...f.value], style: b.value }, [a.value.findIndex((D) => D.key === V.key) + 1])]);
    } });
  };
  te(() => p(re, null, [t.headers ? t.headers(S.value) : c.value.map((_, V) => p("tr", null, [_.map((E, $) => p(y, { column: E, x: $, y: V }, null))])), e.loading && p("tr", { class: "v-data-table-progress" }, [p("th", { colspan: v.value.length }, [p(ro, { name: "v-data-table-progress", active: !0, color: typeof e.loading == "boolean" ? void 0 : e.loading, indeterminate: !0 }, { default: t.loader })])])]));
  ...
  return te(() => {
    ...
    return e.loading && n.loading ? p("tr", { class: "v-data-table-rows-loading", key: "loading" }, [p("td", { colspan: o.value.length }, [n.loading()])]) : e.loading || e.items.length || e.hideNoData ? p(re, null, [e.items.map((h, k) => {
      ...
      const f = { index: k, item: h, columns: o.value, isExpanded: s, toggleExpand: r, isSelected: i, toggleSelect: u }, b = { ...f, props: { key: `item_${h.value}`, onClick: a.value || e["onClick:row"] ? (y) => {
        var _;
        a.value && r(h), (_ = e["onClick:row"]) == null || _.call(e, y, { item: h });
      } : void 0, index: k, item: h } };
      return p(re, null, [n.item ? n.item(b) : p(qs, b.props, n), s(h) && ((S = n["expanded-row"]) == null ? void 0 : S.call(n, f))]);
...
const Ho = M({ ...jo(), width: [String, Number], search: String, ...Mr(), ...Nr(), ...Ws(), ...Ys(), ...jr(), ...Gr(), ...Ro(), ...Go() }, "DataTable"), Js = M({ ...Ma(), ...Ho(), ...Tr(), ...un() }, "VDataTable"), Zs = q()({ name: "VDataTable", props: Js(), emits: { "update:modelValue": (e) => !0, "update:page": (e) => !0, "update:itemsPerPage": (e) => !0, "update:sortBy": (e) => !0, "update:options": (e) => !0, "update:groupBy": (e) => !0, "update:expanded": (e) => !0 }, setup(e, l) {
  let { emit: t, slots: n } = l;
  const { groupBy: o } = Va(e), { sortBy: a, multiSort: r, mustSort: s } = Ua(e), { page: i, itemsPerPage: u } = Ra(e), { columns: d, headers: v } = No(e, { groupBy: o, showSelect: H(e, "showSelect"), showExpand: H(e, "showExpand") }), { items: c } = zo(e, d), m = H(e, "search"), { filteredItems: h } = Lr(e, c, m, { transform: (G) => G.columns }), { toggleSort: k } = qa({ sortBy: a, multiSort: r, mustSort: s, page: i }), { sortByWithGroups: f, opened: b, extractRows: S, isGroupOpen: y, toggleGroup: _ } = Ea({ groupBy: o, sortBy: a }), { sortedItems: V } = zr(e, h, f), { flatItems: E } = Ta(V, o, b), $ = g(() => E.value.length), { startIndex: P, stopIndex: C, pageCount: w, setItemsPerPage: F } = ja({ page: i, itemsPerPage: u, itemsLength: $ }), { paginatedItems: D } = function(G) {
  ...
  }({ items: E, startIndex: P, stopIndex: C, itemsPerPage: u }), B = g(() => S(D.value)), { isSelected: A, select: I, selectAll: L, toggleSelect: N, someSelected: T, allSelected: z } = Wa(e, { allItems: c, currentPage: B }), { isExpanded: j, toggleExpand: W } = Fa(e);
  ...
  const O = g(() => ({ page: i.value, itemsPerPage: u.value, sortBy: a.value, pageCount: w.value, toggleSort: k, setItemsPerPage: F, someSelected: T.value, allSelected: z.value, isSelected: A, select: I, selectAll: L, toggleSelect: N, isExpanded: j, toggleExpand: W, isGroupOpen: y, toggleGroup: _, items: B.value, groupedItems: D.value, columns: d.value, headers: v.value }));
  return te(() => {
  ...
  let { emit: t, slots: n } = l;
  const { groupBy: o } = Va(e), { sortBy: a, multiSort: r, mustSort: s } = Ua(e), { page: i, itemsPerPage: u } = Ra(e), d = g(() => parseInt(e.itemsLength, 10)), { columns: v, headers: c } = No(e, { groupBy: o, showSelect: H(e, "showSelect"), showExpand: H(e, "showExpand") }), { items: m } = zo(e, v), { toggleSort: h } = qa({ sortBy: a, multiSort: r, mustSort: s, page: i }), { opened: k, isGroupOpen: f, toggleGroup: b, extractRows: S } = Ea({ groupBy: o, sortBy: a }), { pageCount: y, setItemsPerPage: _ } = ja({ page: i, itemsPerPage: u, itemsLength: d }), { flatItems: V } = Ta(m, o, k), { isSelected: E, select: $, selectAll: P, toggleSelect: C, someSelected: w, allSelected: F } = Wa(e, { allItems: m, currentPage: m }), { isExpanded: D, toggleExpand: B } = Fa(e), A = g(() => S(m.value));
  ...
  const I = g(() => ({ page: i.value, itemsPerPage: u.value, sortBy: a.value, pageCount: y.value, toggleSort: h, setItemsPerPage: _, someSelected: w.value, allSelected: F.value, isSelected: E, select: $, selectAll: P, toggleSelect: C, isExpanded: D, toggleExpand: B, isGroupOpen: f, toggleGroup: b, items: A.value, groupedItems: V.value, columns: v.value, headers: c.value }));
  te(() => {
...
function Wo(e) {
  ...
  return e.renderer ? e.renderer(...t) : e.renderHeader ? e.renderHeader(...t) : e.renderFooterCell ? e.renderFooterCell(...t) : l || "";
}
function Pl(e) {
...
function kt(e, ...l) {
  ...
  if (Pl(e) && Pl(t))
    for (const n in t)
      Pl(t[n]) ? (e[n] || Object.assign(e, { [n]: {} }), kt(e[n], t[n])) : Object.assign(e, { [n]: t[n] });
  return kt(e, ...l);
}
const ti = (e) => {
...
const ci = M({ dense: Boolean, noGutters: Boolean, align: { type: String, default: null, validator: Xo }, ...Jo, justify: { type: String, default: null, validator: Zo }, ...Qo, alignContent: { type: String, default: null, validator: er }, ...tr, ...se(), ...we() }, "VRow"), lr = q()({ name: "VRow", props: ci(), setup(e, l) {
...
} }), di = ["colspan"], pi = Xe({ __name: "TableLoader", props: { colors: {}, colspan: {}, height: { default: 2 }, level: {}, loaderType: { default: "linear" }, loading: {}, loadingText: { default: "Loading..." }, size: { default: "default" }, skeltonType: {}, textLoader: { type: Boolean, default: !0 } }, setup(e) {
  const l = e, t = cl(), n = g(() => {
    if (typeof l.colors == "object" && l.colors !== null)
      return yl({ colors: l.colors, level: l.level, prop: "loader", themeColors: t });
  }), o = g(() => ((y) => {
    const { isLinearOnly: _, loaderHeight: V } = y;
    let E = {};
    return x(_) && (E = { height: x(V), minHeight: x(V) }), E;
  })({ isLinearOnly: d, loaderHeight: u })), a = g(() => ((y) => {
    const { isLinearOnly: _, loaderHeight: V } = y;
    return x(_) ? { height: x(V), position: "absolute", top: 0, width: "100%" } : {};
    ...
    let _ = !1;
    return y === "linear" && (_ = !0), Array.isArray(y) && (_ = y.length === 1 && y[0] === "linear"), _;
  }(l.loaderType)), v = g(() => {
    var y;
    ...
    const _ = l.loaderType;
    return Array.isArray(_) ? _.indexOf(y) : 1;
  }, S = (y) => {
    const _ = l.loaderType;
    return y === l.loaderType || !!Array.isArray(_) && _.includes(y);
  };
  return (y, _) => {
    const V = ua("v-skeleton-loader");
    return Y(), ie("tr", { class: oe(x(r)), style: $e(x(o)) }, [Re("td", { class: "px-0 ma-0", colspan: y.colspan, style: $e(x(a)) }, [y.loading ? (Y(), me(lr, { key: 0, class: oe(x(s)), "no-gutters": "" }, { default: ae(() => [S("linear") ? (Y(), me(ct, { key: 0, class: "pa-0 ma-0", order: b("linear") }, { default: ae(() => [p(oo, { color: x(i), height: x(u), indeterminate: "" }, null, 8, ["color", "height"])]), _: 1 }, 8, ["order"])) : ye("", !0), S("circular") ? (Y(), me(ct, { key: 1, class: "pa-0 my-2", order: b("circular") }, { default: ae(() => [p(lo, { "bg-color": x(v), color: x(c), indeterminate: "", size: y.size }, null, 8, ["bg-color", "color", "size"])]), _: 1 }, 8, ["order"])) : ye("", !0), S("skelton") ? (Y(), me(ct, { key: 2, class: "pa-0 ma-0", order: b("skelton") }, { default: ae(() => [p(V, { loading: x(h), type: x(m) }, null, 8, ["loading", "type"])]), _: 1 }, 8, ["order"])) : ye("", !0), S("text") ? (Y(), me(ct, { key: 3, class: "my-2", order: b("text"), style: $e(x(k)) }, { default: ae(() => [pt(vr(x(f)), 1)]), _: 1 }, 8, ["order", "style"])) : ye("", !0)]), _: 1 }, 8, ["class"])) : ye("", !0)], 12, di)], 6);
  };
} }), vn = (e, l) => {
  ...
  for (const [n, o] of l)
    t[n] = o;
  return t;
}, Zn = vn(pi, [["__scopeId", "data-v-e1719200"]]), vi = Xe({ __name: "BottomSlot", props: { slotProps: {} }, setup: (e) => (l, t) => pe(l.$slots, "bottom") }), nr = (e) => {
  const { align: l } = e;
  ...
  })({ level: t.level })), v = (f, b = "") => ((S) => {
    const { colors: y, column: _, level: V, slotName: E = "" } = S;
    return { [`${ne}--header-row-th`]: !0, [`${ne}--header-row-th-${E}`]: E !== "", [`${ne}--header-row-th-${E}-${V}`]: E, [`${ne}--header-row-th-${V}`]: !0, [`${ne}--header-row-th-sortable`]: _.sortable, [`${ne}--header-row-th-sortable-default-color`]: _.sortable && y === !1, [`${_.cellClass}`]: _.cellClass };
  })({ colors: t.colors, column: f, level: t.level, slotName: b }), c = (f, b = !1) => ((S) => {
    const { colors: y, column: _, dataTableExpand: V = !1, level: E, theme: $ } = S, P = { minWidth: _.width ? jl(_.width) : "auto", width: _.width ? jl(_.width) : "auto" };
    if (V && !_.width && (P.width = "48px", P.minWidth = "48px"), y === !1 || y === null)
      return P;
    const C = yl({ colors: y, level: E, prop: "header", themeColors: $ });
    return P.backgroundColor = C.bg, P.color = C.text, P;
  ...
  const m = g(() => ar({ level: t.level })), h = (f) => ((b) => {
    const { key: S, level: y, sortBy: _ } = b;
    return { "px-1": !0, [`${ne}--header-row-th-sortable-sort-icon`]: !0, [`${ne}--header-row-th-sortable-sort-icon-${y}`]: !0, [`${ne}--header-row-th-sortable-sort-icon-desc`]: qn(_, S) === "desc", [`${ne}--header-row-th-sortable-sort-icon-asc`]: qn(_, S) === "asc" };
  })({ key: f, level: t.level, sortBy: t.sortBy });
  function k(f) {
    return Wo(f);
  }
  return (f, b) => (Y(), ie("tr", { class: oe(x(d)) }, [(Y(!0), ie(re, null, De(x(s), (S) => {
    return Y(), ie(re, { key: S }, [x(n)[`header.${S.key}`] ? (Y(), ie("th", { key: 0, class: oe(v(S, S.key)), colspan: S.colspan || 1, style: $e(c(S, !0)) }, [pe(f.$slots, `header.${S.key}`, { column: S }, void 0, !0)], 14, fi)) : S.key === "data-table-select" || S.key === "data-table-select" && f.showSelect ? (Y(), ie("th", { key: 1, class: oe(v(S, "header-data-table-select")), colspan: S.colspan || 1, style: $e(c(S, !0)) }, [p(sn, { modelValue: x(a), "onUpdate:modelValue": b[0] || (b[0] = (_) => Ge(a) ? a.value = _ : null), class: oe(x(m)), density: f.density, focused: !1, indeterminate: x(u) }, null, 8, ["modelValue", "class", "density", "indeterminate"])], 14, mi)) : S.key === "data-table-expand" ? (Y(), ie("th", { key: 2, class: oe(v(S, "data-table-expand")), colspan: S.colspan || 1, style: $e(c(S, !0)), innerHTML: k(S) }, null, 14, gi)) : (Y(), ie("th", { key: 3, class: oe(v(S)), colspan: S.colspan || 1, style: $e(c(S)), onClick: (_) => function(V) {
      V.sortable && t.slotProps.toggleSort(V);
    }(S) }, [Re("div", { class: oe((y = S.align, nr({ align: y }))) }, [Re("span", { innerHTML: k(S) }, null, 8, hi), S.sortable && x(n)["header.sortIcon"] ? (Y(), ie("span", { key: 0, class: oe(h(S.key)) }, [pe(f.$slots, "header.sortIcon", {}, void 0, !0)], 2)) : S.sortable ? (Y(), me(he, { key: 1, class: oe(h(S.key)) }, { default: ae(() => [pt(" mdi mdi-arrow-up ")]), _: 2 }, 1032, ["class"])) : ye("", !0)], 2)], 14, yi))], 64);
    var y;
  }), 128))], 2));
} }), Si = vn(bi, [["__scopeId", "data-v-80f3ccaa"]]), wi = ["colspan"], xi = ["colspan"], ki = ["colspan"], Ci = ["colspan"], Bi = ["innerHTML"], _i = Xe({ __name: "ItemSlot", props: { expandOnClick: {}, items: {}, levels: {}, showExpand: {}, slotProps: {}, density: {}, level: {}, showSelect: {} }, emits: ["click:row", "click:row:checkbox", "update:expanded"], setup(e, { emit: l }) {
  const t = e, n = $t(), o = g(() => t.slotProps.columns), a = g(() => t.slotProps.index), r = g(() => t.slotProps.isExpanded), s = g(() => t.slotProps.item), i = g(() => t.slotProps.level), u = g(() => t.slotProps.toggleExpand), d = g(() => t.slotProps.toggleSelect), v = g(() => ((f) => {
    const { expandOnClick: b, level: S, levels: y } = f;
    return { "v-data-table__tr": !0, "v-data-table__tr--clickable": b && S < y, [`${ne}--body-row`]: !0, [`${ne}--body-row-${S}`]: !0 };
  })({ expandOnClick: t.expandOnClick, level: t.level, levels: t.levels })), c = (f) => ((b) => {
    const { column: S, elm: y, level: _ } = b;
    return { [`${ne}--${y}-row-td`]: !0, [`${ne}--${y}-row-td-${_}`]: !0, [`${S.cellClass}`]: S.cellClass };
  })({ column: f, elm: "body", level: t.level });
  function m(f) {
    const { item: b, level: S, toggleExpand: y } = f;
    if (!(t.level >= t.levels)) {
      if (f.$event) {
        if (!t.expandOnClick)
          return;
      ...
      S === t.level && y(b), l("update:expanded", f);
    }
  }
  const h = U(!1);
  function k(f, b) {
    return function(S, y) {
      const _ = S[y.key];
      return y.renderItem ? y.renderItem(_, S, y) : _;
    }(f.raw, b);
  }
  return J(() => t.slotProps.allRowsSelected, () => {
    h.value = t.slotProps.allRowsSelected;
  }), (f, b) => (Y(), ie("tr", { class: oe(x(v)), onClick: b[3] || (b[3] = (S) => m({ columns: x(o), index: x(a), isExpanded: x(r), item: x(s), level: x(i), toggleExpand: x(u), $event: S })) }, [(Y(!0), ie(re, null, De(x(o), (S) => (Y(), ie(re, { key: S }, [f.$slots[`item.${S.key}`] ? (Y(), ie("td", { key: 0, class: oe(c(S)), colspan: S.colspan || 1 }, [pe(f.$slots, `item.${S.key}`, { columns: x(o), index: x(a), item: x(s) }, void 0, !0)], 10, wi)) : S.key === "data-table-select" || S.key === "data-table-select" && t.showSelect ? (Y(), ie("td", { key: 1, class: oe(c(S)), colspan: S.colspan || 1 }, [x(n)["item.data-table-select"] ? pe(f.$slots, "item.data-table-select", { key: 0 }, void 0, !0) : ye("", !0), p(sn, { modelValue: x(h), "onUpdate:modelValue": b[0] || (b[0] = (y) => Ge(h) ? h.value = y : null), class: "d-flex v-simple-checkbox", density: f.density, onClick: b[1] || (b[1] = (y) => function(_) {
    const { item: V, level: E, toggleSelect: $ } = _;
    E === t.level && $(V), l("click:row:checkbox", V);
  }({ columns: x(o), index: x(a), item: x(s), level: x(i), toggleSelect: x(d) })) }, null, 8, ["modelValue", "density"])], 10, xi)) : S.key === "data-table-expand" || S.key === "data-table-expand" && f.showExpand ? (Y(), ie("td", { key: 2, class: oe(c(S)), colspan: S.colspan || 1 }, [x(i) < f.levels ? (Y(), ie("div", { key: 0, class: oe(["v-drilldown-table--expand-icon", x(r)(x(s)) ? "rotate-180" : ""]), onClick: b[2] || (b[2] = (y) => m({ columns: x(o), index: x(a), isExpanded: x(r), item: x(s), level: x(i), toggleExpand: x(u) })) }, [f.$slots["item.data-table-expand"] ? pe(f.$slots, "item.data-table-expand", { key: 0 }, void 0, !0) : (Y(), me(he, { key: 1 }, { default: ae(() => [pt(" mdi-chevron-down ")]), _: 1 }))], 2)) : ye("", !0)], 10, ki)) : (Y(), ie("td", { key: 3, class: oe(c(S)), colspan: S.colspan || 1 }, [Re("span", { innerHTML: k(x(s), S) }, null, 8, Bi)], 10, Ci))], 64))), 128))], 2));
} }), Pi = vn(_i, [["__scopeId", "data-v-12e9f83e"]]), Fi = ["colspan"], Ai = ["colspan"], $i = ["colspan", "innerHTML"], Vi = ["colspan", "innerHTML"], Ei = ["colspan"], Ii = ["innerHTML"], Oi = Xe({ __name: "TfootSlot", props: { footers: {}, slotProps: {}, colors: {}, density: {}, level: {} }, emits: ["click:selectAll"], setup(e, { emit: l }) {
  ...
  })({ level: t.level })), c = (f, b = "") => ((S) => {
    const { column: y, level: _, slotName: V = "" } = S;
    return { [`${ne}--tfoot-row-td`]: !0, [`${ne}--tfoot-row-td-${V}`]: V !== "", [`${ne}--tfoot-row-td-${V}-${_}`]: V, [`${ne}--tfoot-row-td-${_}`]: !0, [`${y.cellClass}`]: y.cellClass };
  })({ column: f, level: t.level, slotName: b }), m = g(() => ((f) => {
    const { colors: b, elm: S, level: y, theme: _ } = f;
    if (b === !1 || b === null)
      return {};
    const V = yl({ colors: b, level: y, prop: S, themeColors: _ });
  ...
  return (f, b) => {
    var S;
    return (S = x(s)) != null && S.length ? (Y(), ie("tfoot", { key: 0, class: oe(x(d)) }, [Re("tr", { class: oe(x(v)) }, [(Y(!0), ie(re, null, De(x(s), (y) => {
      return Y(), ie(re, { key: y }, [x(n)[`tfoot.${y.key}`] ? (Y(), ie("th", { key: 0, class: oe(c(y, y.key)), colspan: y.colspan || 1, style: $e(x(m)) }, [pe(f.$slots, `tfoot.${y.key}`, { column: y })], 14, Fi)) : y.key === "data-table-select" ? (Y(), ie("th", { key: 1, class: oe(c(y, "tfoot-data-table-select")), colspan: y.colspan || 1, style: $e(x(m)) }, [p(sn, { modelValue: x(a), "onUpdate:modelValue": b[0] || (b[0] = (V) => Ge(a) ? a.value = V : null), class: oe(x(h)), density: f.density, focused: !1, indeterminate: x(u) }, null, 8, ["modelValue", "class", "density", "indeterminate"])], 14, Ai)) : y.key === "data-table-expand" ? (Y(), ie("th", { key: 2, class: oe(c(y, "tfoot-data-table-expand")), colspan: y.colspan || 1, style: $e(x(m)), innerHTML: k(y) }, null, 14, $i)) : y.renderFooterCell ? (Y(), ie("th", { key: 3, class: oe(c(y)), colspan: y.colspan || 1, style: $e(x(m)), innerHTML: k(y) }, null, 14, Vi)) : (Y(), ie("th", { key: 4, class: oe(c(y)), colspan: y.colspan || 1, style: $e(x(m)) }, [Re("div", { class: oe((_ = y.align, nr({ align: _ }))) }, [Re("span", { innerHTML: k(y) }, null, 8, Ii)], 2)], 14, Ei))], 64);
      var _;
    }), 128))], 2)], 2)) : ye("", !0);
  };
} });
...
const wt = /* @__PURE__ */ new Map(), Hi = Xe({ __name: "TopSlot", props: { items: {}, level: {}, loading: {}, searchProps: { default: () => ({ cols: { lg: 3, md: 6, sm: 12, xl: 3, xs: 12, xxl: 2 }, density: "comfortable", variant: "underlined" }) }, showSearch: { type: Boolean }, slotProps: {} }, emits: ["click:selectAll", "update:search"], setup(e, { emit: l }) {
  ...
  const m = (w) => {
    ...
    return !(w && (F === "skelton" || Array.isArray(F) && F.length === 1 && F[0] === "skelton" || !a.showDrilldownWhenLoading));
  }, h = g(() => ((w) => {
    const { elevation: F, isDrilldown: D, isHover: B, isServerSide: A, level: I } = w;
    return { "pb-2": !0, [`${ne}--child`]: D, [`${ne}--hover`]: B, [`${ne}--level-${I}`]: !0, [`${ne}--server`]: A, [`${ne}`]: !0, [`elevation-${F}`]: parseInt(F) > 0 };
  })({ elevation: a.elevation, isDrilldown: t.isDrilldown, isHover: a.hover, isServerSide: !1, level: a.level })), k = g(() => ((w) => {
    const { colors: F, level: D, theme: B } = w;
    let A = {};
  ...
  function f() {
    ...
    })({ drilldown: t.drilldown, level: t.level, levels: t.levels, loadedDrilldown: a, rawItem: (w = t.item) == null ? void 0 : w.raw }) : kt(a, t);
  }
  function b(w) {
    s.value = w;
  }
  function S(w) {
    ...
    t.server && (F = { ...a, ...r, ...w }), ((D) => {
      ...
      B.isExpanded(B.item) && I("update:drilldown", A), I("update:expanded", B);
    })({ data: w, drilldownData: F, emit: l });
  }
  function y(w) {
    return { items: w.items, itemsPerPage: w.itemsPerPage, page: w.page, search: u.value, server: w.server, sortBy: w.sortBy };
  }
  const _ = function(w) {
    const F = fr();
    function D(I) {
      var L;
      const N = wt.get(w) || /* @__PURE__ */ new Set();
      N.add(I), wt.set(w, N);
      const T = () => B(I);
    ...
    return { on: D, once: function(I) {
      ...
      var N;
      (N = wt.get(w)) == null || N.forEach((T) => T(I, L));
    }, reset: A };
  }(Wi), V = _.on(function(w) {
    t.level === 1 && w.drilldown.server && l("update:options", { ...w });
  });
  function E(w) {
    a.itemsPerPage = w;
    const F = y(a), D = { drilldown: { ...t, ...F, itemsPerPage: w }, itemsPerPage: w, name: "update:itemsPerPage" };
    _.emit(D);
  }
  function $(w) {
    a.page = w;
    const F = y(a), D = { drilldown: { ...t, ...F, page: w }, name: "update:page", page: w };
    _.emit(D);
  }
  function P(w) {
    a.sortBy = w;
    const F = y(a), D = { drilldown: { ...t, ...F, sortBy: w }, name: "update:sortBy", sortBy: w };
    _.emit(D), l("update:sortBy", w);
  }
  function C() {
  }
  return gr(() => {
    ...
    a.search = u.value;
    const w = y(a), F = { drilldown: { ...t, ...w, search: u.value }, name: "update:search", search: u.value };
    _.emit(F);
  }, { debounce: a.searchDebounce, maxWait: a.searchMaxWait }), (w, F) => {
    const D = ua("VDrilldownTable", !0);
    return x(o) ? (Y(), me(ia(x(o)), Z({ key: 0 }, w.$attrs, { class: x(h), density: x(a).density, "expand-on-click": x(a).expandOnClick, expanded: x(a).expanded, headers: x(a).headers, height: x(a).height, "hide-no-data": x(c), "item-value": x(a).itemValue, items: x(a).items, "items-length": x(a).itemsLength, "items-per-page": x(a).itemsPerPage, "items-per-page-options": x(a).itemsPerPageOptions, loading: (!x(a).loaderType || x(n).loading) && x(a).loading, "multi-sort": x(a).multiSort, "must-sort": x(a).mustSort, "no-data-text": x(a).noDataText, "no-filter": x(a).noFilter, page: x(a).page, "return-object": x(a).returnObject, search: x(u), "show-expand": x(a).showExpand, "sort-by": x(v), style: x(k), "onUpdate:itemsPerPage": E, "onUpdate:modelValue": C, "onUpdate:page": $, "onUpdate:sortBy": P }), et({ top: ae((B) => [(Y(), me(x(Hi), { key: w.level, items: x(a).items, level: x(a).level, loading: x(a).loading, "search-props": x(a).searchProps, "show-search": x(a).showSearch ?? !1, "slot-props": B, "onClick:selectAll": F[0] || (F[0] = (A) => b(A)), "onUpdate:search": F[1] || (F[1] = (A) => u.value = A) }, et({ _: 2 }, [De(x(n), (A, I) => ({ name: I, fn: ae((L) => [pe(w.$slots, I, _e(Ne({ ...L })))]) }))]), 1032, ["items", "level", "loading", "search-props", "show-search", "slot-props"]))]), headers: ae((B) => [(Y(), me(x(Si), { key: w.level, colors: x(a).colors, density: x(a).density, level: w.level, "show-select": x(a).showSelect, "slot-props": { allRowsSelected: x(s), ...B }, "sort-by": x(a).sortBy, "onClick:selectAll": F[2] || (F[2] = (A) => b(A)) }, et({ _: 2 }, [De(x(n), (A, I) => ({ name: I, fn: ae((L) => [pe(w.$slots, I, _e(Ne({ ...L })))]) }))]), 1032, ["colors", "density", "level", "show-select", "slot-props", "sort-by"])), x(a).loading && x(a).loaderType && !x(n).loading && w.level === 1 ? (Y(), me(x(Zn), { key: 0, colors: x(a).colors || null, colspan: B.columns.length, height: x(a).loaderHeight, level: x(a).level, "loader-type": x(a).loaderType, loading: x(a).loading || !1, "loading-text": w.loadingText, "skelton-type": x(a).skeltonType }, null, 8, ["colors", "colspan", "height", "level", "loader-type", "loading", "loading-text", "skelton-type"])) : ye("", !0)]), item: ae((B) => [(Y(), me(x(Pi), { key: w.level, density: x(a).density, "expand-on-click": x(a).expandOnClick, items: x(a).items, level: x(a).level, levels: x(a).levels, "show-expand": x(a).showExpand, "show-select": x(a).showSelect, "slot-props": { allRowsSelected: x(s), level: w.level, ...B }, "onClick:row": F[3] || (F[3] = (A) => {
      l("click:row", A);
    }), "onClick:row:checkbox": F[4] || (F[4] = (A) => {
      l("click:row:checkbox", A);
    }), "onUpdate:expanded": F[5] || (F[5] = (A) => S(A)) }, et({ _: 2 }, [De(x(n), (A, I) => ({ name: I, fn: ae((L) => [pe(w.$slots, I, _e(Ne({ ...L })))]) }))]), 1032, ["density", "expand-on-click", "items", "level", "levels", "show-expand", "show-select", "slot-props"]))]), "expanded-row": ae(({ columns: B, item: A }) => {
      var I, L, N, T, z, j, W, O, G, R;
      return [(I = A.raw[w.itemChildrenKey]) != null && I.loading && x(a).loaderType && !x(n).loading ? (Y(), me(x(Zn), { key: 0, class: "pa-0 ma-0", colors: ((L = A.raw[w.itemChildrenKey]) == null ? void 0 : L.colors) ?? null, colspan: B.length, height: A.raw[w.itemChildrenKey].loaderHeight, level: w.level + 1, "loader-type": A.raw[w.itemChildrenKey].loaderType, loading: (N = A.raw[w.itemChildrenKey]) == null ? void 0 : N.loading, "loading-text": w.loadingText, "skelton-type": A.raw[w.itemChildrenKey].skeltonType }, null, 8, ["colors", "colspan", "height", "level", "loader-type", "loading", "loading-text", "skelton-type"])) : ye("", !0), Re("tr", { class: oe(m((T = A.raw[w.itemChildrenKey]) == null ? void 0 : T.loading) ? "" : "d-none") }, [Re("td", { class: "px-0 ma-0", colspan: B.length, style: { "vertical-align": "top" } }, [(Y(), me(D, { key: A.raw, colors: w.colors, drilldown: x(a), headers: (z = A.raw[w.itemChildrenKey]) == null ? void 0 : z.headers, "is-drilldown": !0, item: A, "items-length": (j = A.raw[w.itemChildrenKey]) == null ? void 0 : j.itemsLength, "items-per-page": (W = A.raw[w.itemChildrenKey]) == null ? void 0 : W.itemsPerPage, level: w.level + 1, levels: x(a).levels, loading: (O = A.raw[w.itemChildrenKey]) == null ? void 0 : O.loading, "multi-sort": (G = A.raw[w.itemChildrenKey]) == null ? void 0 : G.multiSort, "no-data-text": x(a).noDataText, "parent-ref": x(i), server: (R = A.raw[w.itemChildrenKey]) == null ? void 0 : R.server, "sort-by": x(a).sortBy, "table-type": x(o), "onUpdate:drilldown": F[6] || (F[6] = (K) => S(K)), "onUpdate:modelValue": C }, et({ default: ae(() => [(Y(!0), ie(re, null, De(Object.keys(x(n)), (K) => pe(w.$slots, K)), 256))]), _: 2 }, [De(x(n), (K, le) => ({ name: le, fn: ae((ee) => [pe(w.$slots, le, _e(Ne({ ...ee })))]) }))]), 1032, ["colors", "drilldown", "headers", "item", "items-length", "items-per-page", "level", "levels", "loading", "multi-sort", "no-data-text", "parent-ref", "server", "sort-by", "table-type"]))], 8, Ki)], 2)];
    }), "footer.prepend": ae(() => [x(n)["footer.prepend"] ? pe(w.$slots, "footer.prepend", { key: 0 }) : ye("", !0)]), _: 2 }, [x(n).loading ? { name: "loading", fn: ae(() => [pe(w.$slots, "loading")]), key: "0" } : void 0, x(n).thead ? { name: "thead", fn: ae((B) => [pe(w.$slots, "thead", _e(Ne({ ...B })))]), key: "1" } : void 0, x(n).body ? { name: "body", fn: ae((B) => [pe(w.$slots, "body", _e(Ne({ ...B })))]), key: "2" } : void 0, x(n).tbody ? { name: "tbody", fn: ae((B) => [pe(w.$slots, "tbody", _e(Ne({ ...B })))]), key: "3" } : void 0, x(n)["no-data"] ? { name: "no-data", fn: ae(() => [pe(w.$slots, "no-data")]), key: "4" } : void 0, x(n).tfoot || w.showFooterRow ? { name: "tfoot", fn: ae((B) => [x(n).tfoot ? pe(w.$slots, "tfoot", _e(Z({ key: 0 }, { ...B }))) : (Y(), me(x(Oi), { key: w.level, colors: x(a).colors || null, density: x(a).density, footers: x(a).footers || [], level: x(a).level, "show-select": x(a).showSelect, "slot-props": { allRowsSelected: x(s), ...B } }, et({ _: 2 }, [De(x(n), (A, I) => ({ name: I, fn: ae((L) => [pe(w.$slots, I, _e(Ne({ ...L })))]) }))]), 1032, ["colors", "density", "footers", "level", "show-select", "slot-props"]))]), key: "5" } : void 0, x(n).bottom ? { name: "bottom", fn: ae((B) => [(Y(), me(x(vi), { key: w.level, "slot-props": B }, et({ _: 2 }, [De(x(n), (A, I) => ({ name: I, fn: ae((L) => [pe(w.$slots, I, _e(Ne({ ...L })))]) }))]), 1032, ["slot-props"]))]), key: "6" } : void 0]), 1040, ["class", "density", "expand-on-click", "expanded", "headers", "height", "hide-no-data", "item-value", "items", "items-length", "items-per-page", "items-per-page-options", "loading", "multi-sort", "must-sort", "no-data-text", "no-filter", "page", "return-object", "search", "show-expand", "sort-by", "style"])) : ye("", !0);
  };
} });
oa.install = (e) => {
